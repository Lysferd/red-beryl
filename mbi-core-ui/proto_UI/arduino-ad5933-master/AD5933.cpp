...
bool AD5933::setStartFrequency(unsigned long start) {
	...
	/*unsigned long tempValue = clockSpeed/4;
	tempValue = start/tempValue;
	unsigned long freqHex = tempValue*(pow(2,27));
	*/
    unsigned long freqHex = (start / (clockSpeed / 4.0))*(pow(2, 27));
    if (freqHex > 0xFFFFFF) {
